...
// - [ ] change the c# written lsp to avoid function in comments and strings
// - [ ] after closing the dialog you need to clear the lsp document symbols

// # Developer Tools
// -> Performance tab
//
// What is cumulative layout shift (CLS) 3.09 "your local CLS value of 3.09 is poor" worst cluster 25 shifts

/*
Google AI Overview "vanilla js virtualization: when scrolling down should I move the first div to the last div and swap out the innerText":
#```paraphrased
Yes, moving the first DOM element to the end of the list (and updating its content/position) is exactly how high-performance "recycling" or "windowing" works in Vanilla JS.
This technique keeps memory usage and DOM node count low, ensuring smooth scrolling even with thousands of items.

...

The Problem with this Approach (And the Better Way)
While recycling and moving DOM nodes works, simply moving the first element to the last position can cause layout thrashing and visual flickering,
especially if the user scrolls quickly.

...

...Instead of moving nodes around in the DOM tree, you simply
update the innerText and
adjust the style.transform (or top property) of the visible nodes to place them in the correct Y coordinate.
...
I've been messing with things but I had an idea to look this up:

Google AI Overview "javascript html dom api to swap the position of two child nodes without causing layout":
#```
...

Why this avoids layout shifts

Both replaceChild() and insertBefore() are optimized by modern browsers to update the DOM tree's pointers instantly without causing a browser reflow or repaint,
provided the dimensions (width/height) of the swapped nodes remain identical.

If you are swapping nodes that have different styles or dimensions,
would you like tips on how to use translate transformations to prevent visual layout shifts on the page?
#```
